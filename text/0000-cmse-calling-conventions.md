...
The cmse calling conventions facilitate interactions between the secure and non-secure executables. To ensure that secrets do not leak, these calling conventions impose some custom restrictions on top of the system's standard AAPCS calling convention: the stack may not be used for passing arguments or return values, so calls are restricted to 4 registers for passing arguments, and 1 register for passing the return value. The stack is never used for argument passing. The compiler checks that the signature is valid.
...
The link step adds an additional `.gnu.sgstubs` section to the binary, which contains a veneer (shim in rust terminology) that first calls the `sg` instruction, switching to the secure state. It then branches to the actual function it veneers:
...
This ABI is only accepted on `thumbv8m` targets, on all other targets it generates an invalid ABI error. In terms of passing arguments and return values, it is based on AAPCS, but imposes additional restrictions.
...
The secure executable can get its hands on a non-secure function pointer in two ways: it can be an argument to an entry function, or the function can be stored at a statically-known address.
